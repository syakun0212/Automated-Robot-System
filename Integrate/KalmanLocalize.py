...
class KalmanLocalizerNode(object):
    ...
    def getMeasurement(self):
        return np.array([[self.stateXDotRead], [self.stateWRead], [self.imuXAcc], [self.imuYAcc], [self.imuW], [0.0], [0.0], [0.0]])
    
    def getMeasurementNoise(self):
        self.measurementNoise = np.power(np.array([0.1, 0.2, 0.2*9.81, 0.2*9.81, 0.015, 1000.0, 1000.0, 1000.0]),2)
    ...
    def localizeLoop(self, delay=0.02):
        ...
        while(True):
            ...
            self.predictNoise = matF @ self.predictNoise @ matF.T + Q(dt, accXStd, accYStd, wStd)
