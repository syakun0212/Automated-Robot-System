...
class Map(object):
    ...
    def plotKeypoints(self, xlim=(0, 2.0), ylim=(0, 2.0), quiverScale=20, newFig=False):
        if(newFig):
            fig = plt.figure()
...
class ParticleFilter(object):
    ...
    def resampleWeight(self): 
        index = np.random.choice(self.N, p=self.weights, size=self.N)
        self.particles = self.particles[:,index]
        self.initWeight()

    def resample(self): 
        mean, var = self.getCurrentEstimate()
        self.particles = gaussianParticles(mean, np.sqrt(var), self.N)
        self.initWeight()
...
def plotParticles(particles, xlim=[0,2], ylim=[0,2], newFig=False):
    if newFig:
        fig = plt.figure()
...
class ParticleFilterNode(object):
    ...
    def __init__(self, N, camMatrix):
        ...
        self.startPose = np.array([[0.2],[0.2],[90*math.pi/180]])
        ...
        obstacleSize = (0.1,0.1)
        obstacleCount = 41

        self.obstaclesInput = [
            ObstacleData((0.2, 1.8, math.pi/2), obstacleSize, (-1, 0, 0, 0)),
            ObstacleData((0.8, 1.0, -math.pi), obstacleSize, (-1, 0, 0, 0)),
            ObstacleData((1.2, 0.5, 0), obstacleSize, (-1, 0, 0, 0)),
        ]
        self.detectionBank = [[0 for i in range(obstacleCount)] for j in range(3)]
    ...
    def loop(self):
        ...
        try:
            while (True):
                ...
                if(poseCount >= POSE_UPDATE_COUNT):
                    ...
                    fig.gca().clear()
                    self.map.plotKeypoints()
                    plotParticles(self.pf.particles)
                    plotPose(np.expand_dims(np.expand_dims(curEst,0),-1))
                    plt.legend(["Particles", "Particles Heading", "Estimate Pose"])
                    plt.pause(0.002)

                    tVert,worldV,ind = self.map.getProjection(np.expand_dims(curEst, 0), self.projMatrix)
                    if(len(tVert.shape) == 3):
                        tVertN = np.divide(tVert, np.expand_dims(tVert[:,2,:],1))
                        points.append((time.time() - startTime, tVertN, ind))
        ...
        with open("projections.p", 'wb') as f:
...
def main():
    with open("new_camera_calib_pickle.p", "rb" ) as f:
        calib_result_pickle = pickle.load(f)
